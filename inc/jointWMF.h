...
public:
    ...
    static Mat filter(Mat &I, Mat &feature, int r, float sigma = 25.5, int nI = 256, int nF = 256, int iter = 1,
                      string weightType = "exp", Mat mask = Mat()) {
...
private:
    ...
    static void featureIndexing(Mat &F, float **&wMap, int &nF, float sigmaI, string weightType) {
        ...
        if (F.channels() == 1) {
            ...
            Mat centers;
            {
                kmeans(samples, nF, labels, TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 0, 10000), KmeansAttempts,
            ...
            // Computer weight map (weight between each pair of feature index)
            {
                ...
                for (int i = 0; i < nF; i++) {
                    for (int j = i; j < nF; j++) {
                        ...
                        if (op == 0)
                            wMap[i][j] = wMap[j][i] = exp(
                                    -(diff0 * diff0 + diff1 * diff1 + diff2 * diff2) * divider); // EXP 2
                        else if (op == 2)
                            wMap[i][j] = wMap[j][i] =
                                    1.0f / (diff0 * diff0 + diff1 * diff1 + diff2 * diff2 + nSigmaI * nSigmaI); // IV2
                        else if (op == 1)
                            ...
                            wMap[i][j] = wMap[j][i] = (min(a0, b0) + min(a1, b1) + min(a2, b2)) /
                                                      (max(a0, b0) + max(a1, b1) + max(a2, b2)); // Jacard
                        else if (op == 5)wMap[i][j] = wMap[j][i] = 1.0f; // Unweighted
                    }
                }
